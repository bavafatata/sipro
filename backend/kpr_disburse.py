...
async def assign_scheme(org: str, contract: dict, app: dict, scheme_id: str, actor: str,
    ...
    if sch.get("is_active") is False:
        raise ValueError(f"Skema pencairan '{sch.get('name')}' sudah nonaktif — pilih skema lain.")
    ...
    if sch.get("bank") and app.get("bank_name") and sch["bank"] != app.get("bank_name") and not auto:
        raise ValueError(f"Skema '{sch['name']}' khusus bank {sch['bank']}, sedangkan pengajuan ini bank "
                         f"{app.get('bank_name')} — pilih skema bank yang sesuai atau skema 'semua bank'.")
    ...
    await db.financing_apps.update_one({"id": app["id"]}, {"$set": {
        ...
        "disbursement_scheme_code": sch.get("code"), "disbursement_scheme_auto": bool(auto),
    ...
    await add_activity(entity_type="customer", entity_id=contract.get("customer_id"), type="system",
                       ...
                       body=(f"Skema pencairan KPR '{sch['name']}' {'dipasang otomatis (bawaan konfigurasi)' if auto else 'dipilih'}"
                             f" — {len(tranches)} tahap dari plafon {_rp(plafon)}."))
    return await db.financing_apps.find_one({"id": app["id"]}, {"_id": 0})


async def default_scheme(org: str, bank: str = None) -> dict:
    """Skema bawaan dari `[CFG] kpr.disbursement.default_scheme_code`; skema khusus bank pengajuan
    (aktif, satu-satunya) diprioritaskan bila ada."""
    import settings_store as cfg
    if bank:
        rows = await db.kpr_disbursement_schemes.find({"org_id": org, "bank": bank, "is_active": True},
                                                      {"_id": 0}).to_list(10)
        if len(rows) == 1:
            return rows[0]
    code = str(await cfg.get("kpr.disbursement.default_scheme_code", org_id=org) or DEFAULT_SCHEME["code"])
    await ensure_default(org)
    return await db.kpr_disbursement_schemes.find_one({"org_id": org, "code": code.upper(), "is_active": True},
                                                      {"_id": 0}) \
        or await db.kpr_disbursement_schemes.find_one({"org_id": org, "code": DEFAULT_SCHEME["code"]}, {"_id": 0})


async def ensure_scheme(org: str, contract: dict, app: dict, actor: str) -> dict:
    """Pasang skema bawaan bila kontrak KPR belum punya tahapan (dipanggil saat SP3K & pencairan)."""
    if app.get("tranches") or int(app.get("approved_plafon") or 0) <= 0:
        return app
    sch = await default_scheme(org, app.get("bank_name"))
    if not sch:
        return app
    return await assign_scheme(org, contract, app, sch["id"], actor, auto=True)
...
async def disburse(org: str, contract: dict, app: dict, payload: dict, user: dict) -> dict:
    ...
    # `[CFG] kpr.disbursement.require_scheme`: tanpa tahapan → pasang skema bawaan; bila tetap tidak
    # ada (plafon kosong) dan aturan menyala → nominal bebas DITOLAK.
    import settings_store as cfg
    require_scheme = bool(await cfg.get("kpr.disbursement.require_scheme", org_id=org))
    if not app.get("tranches"):
        app = await ensure_scheme(org, contract, app, actor)
    tranches = app.get("tranches") or []
    if not tranches and require_scheme:
        raise ValueError("Pencairan wajib mengikuti SKEMA PENCAIRAN (konfigurasi `kpr.disbursement.require_scheme` "
                         "menyala) — pastikan plafon SP3K tercatat lalu pilih skema pencairan pada kontrak.")
    tranche = None
    if tranches:
