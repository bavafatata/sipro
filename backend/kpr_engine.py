...
async def kpr_advance(org: str, contract_id: str, stage: str, payload: dict,
    ...
    if stage == "sp3k":
        ...
        if int(payload.get("plafon") or 0) <= 0:
            raise ValueError("Plafon yang DISETUJUI bank wajib diisi (lebih dari 0) pada tahap SP3K.")
        bd_sp3k = await _ce().build_breakdown(org, c)
        harga = int(bd_sp3k.get("net_price") or bd_sp3k.get("gross_price") or 0)
        if harga and int(payload["plafon"]) > harga:
            raise ValueError(f"Plafon {_rp(payload['plafon'])} melebihi harga bersih unit {_rp(harga)} — "
                             "plafon KPR tidak boleh lebih besar dari kewajiban pembeli.")
    ...
    # SP3K = plafon pasti → skema pencairan BAWAAN (Pusat Konfigurasi › Pencairan KPR) dipasang
    # otomatis bila belum dipilih, sehingga tahapan pencairan selalu ada saat finance mencatat.
    if stage == "sp3k":
        import kpr_disburse as kd
        fresh = await db.financing_apps.find_one({"id": app["id"]}, {"_id": 0})
        try:
            await kd.ensure_scheme(org, c, fresh, actor)
        except ValueError as e:  # skema bawaan tidak cocok → biarkan sales/finance memilih manual
            logger.warning("Skema pencairan bawaan tidak dipasang: %s", e)
